'''
Bouaziz Imene ISI G01 
Robotics Homework : A* Path Finder Algoritm

To use the program:
Run the code, Once the Pygame window appears : 
1. Left-click to place the start, end, then barriers.
2. Right-click to remove barriers.
3. Press the spacebar to run the A* algorithm after placing the start and end.
4. Press 'C' to clear the grid and start over.
5. Close the Pygame window to exit the program.
...
# Get the row and column clicked by the mouse
...
# Main function to set up the Pygame window and handle user interactions
def main(win, width):
    ROWS = 50
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            # Left-click to place start, end, or barriers
